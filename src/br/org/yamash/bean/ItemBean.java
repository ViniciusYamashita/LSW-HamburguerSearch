...
public class ItemBean {
	...
	public void alterar() {
		try {
			...
			facesUntil
					.adicionarMsgErro("Ocorreu um erro ao Alterar o Item. Favor contatar o administrador do sistema!");
		}
	}
	...
	public void alterarStatus(Item item) {
		try {
			...
			facesUntil.adicionarMsgErro(
					"Ocorreu um erro ao Alterar a situação do Item. Favor contatar o administrador do sistema!");
		}
	}

	public void listarItemDaEmpresa() {
		...
		try {
			if (valor != null) {
				...
				ItemDAO itemDAO = new ItemDAO();
				itens = itemDAO.filtrarPorEmpresa(codigo);
			}
		} catch (RuntimeException ex) {
			facesUntil.adicionarMsgErro(
